...
export function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex flex-col justify-center items-center p-4 font-onest">
      <div className={`container md:py-3 md:px-8 transition-all duration-300 md:backdrop-blur-lg md:border md:border-brand-white/20 w-full max-w-7xl rounded-full ${
        isScrolled ? "md:bg-brand-white md:shadow-lg text-foreground" : "text-brand-white"
      }`}
      >
          ...
          <div className="w-full flex items-center gap-3 md:justify-end">
            ...
            >
              ...
              <span className={`relative block w-5 h-3.5 transition-all`}>
                <span className={`absolute left-0 top-0 h-0.5 w-5 bg-foreground transition-transform rounded-full ${isMenuOpen ? 'translate-y-1.5 rotate-45' : ''}`}></span>
                <span className={`absolute left-0 top-1.5 h-0.5 w-5 bg-foreground transition-opacity rounded-full ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
                <span className={`absolute left-0 bottom-0 h-0.5 w-5 bg-foreground transition-transform rounded-full ${isMenuOpen ? '-translate-y-1.5 -rotate-45' : ''}`}></span>
              </span>
            </button>
